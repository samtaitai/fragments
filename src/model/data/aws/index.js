...
async function listFragments(ownerId, expand = false) {
  ...
  // we'll get back every attribute(item).  The projection expression defines a list
  // of attributes to return, see:
  // https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Expressions.ProjectionExpressions.html
  // To retrieve a single attribute('id' in this case), specify its name. 
  ...
  try {
    ...
    // no expand => return ids, expand => return items
...
async function deleteFragment(ownerId, id) {
  ...
  const commandForMeta = new DeleteCommand(paramsForMeta);
  ...
  try {
    await ddbDocClient.send(commandForMeta);
  } catch (err) {
    const { TableName, Key } = paramsForMeta;
    logger.error({ err, TableName, Key }, 'Error streaming fragment data from DynamoDB');
